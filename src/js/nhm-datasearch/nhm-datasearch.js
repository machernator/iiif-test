...
function init(el) {
	...
	// Prevent usage of enter key, form will not be sent
	fields[searchid].searchField.addEventListener('keydown', function (e) {
		// handle enter key
		if (e.key === "Enter") {
			e.preventDefault();
			return;
	...
	fields[searchid].results.addEventListener('click', function (e) {
		const t = e.target;
		const id = t.closest('[data-idprefix]').dataset.idprefix;

		// some lis, like no result found, should not be selectable
		const li = t.closest('li');
		if (li && li.dataset.noclick) {
			e.preventDefault();
			return false;
		}
...
function updateResults(field, data) {
	...
	if (data.length == 0) {
		...
		li.innerText = "No results were found.";
		li.setAttribute("data-noclick", true);
		frag.appendChild(li);
	} else {
		for (const key in data) {
			...
			if (entry.count !== undefined) {
				...
				counter.className = "badge bg-secondary px-2 py-1 ms-3 rounded-pill";
				counter.innerText = entry.count;
...
window.nhmDataSearch = nhmDataSearch;
